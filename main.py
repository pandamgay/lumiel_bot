...
전체적으로 일관성있게
딱 읽기좋게
쿼리도 싹다 줄바꿈 잘 하셈                             # utill 모듈로 뻄
로그 구조도 확실하게 하나로 맞춰서                     # utill 모듈로 뻄
함수 시작할 때 시작 로그                               # utill 모듈로 뻄
중간에 에러나면 예외에다 에러났다고 응답하고 로그      # utill 모듈로 뻄
성공 실패 있는 명령어면 성공실패 로그에 찍고 이유      # utill 모듈로 뻄
시작할 때 \n찍어서 매개변수 보여주면 됨                # utill 모듈로 뻄
로그가 중요함 레벨도 딱 정확한 기준을 잡고 리팩터링 해야함
내용도 그렇고
일단 utill로 빼는것도 스타일은 다 리팩터링 하고, 그때 재사용성 보는게 더 편할듯
'''
